...
class Hyperparameters:
    data_path = resolve_repo_path(os.environ.get("DATA_PATH", "data/datasets/fineweb10B_sp1024"))
    train_files = os.path.join(data_path, "fineweb_train_*.bin")
    val_files = os.path.join(data_path, "fineweb_val_*.bin")
    tokenizer_path = resolve_repo_path(os.environ.get("TOKENIZER_PATH", "data/tokenizers/fineweb_1024_bpe.model"))
    ...
    bigram_prior_path = resolve_repo_path(os.environ.get("BIGRAM_PRIOR_PATH", None))
    ...
    unigram_probs_path = resolve_repo_path(os.environ.get("UNIGRAM_PROBS_PATH", None))
    ...
    lowrank_bigram_path = resolve_repo_path(os.environ.get("LOWRANK_BIGRAM_PATH", None))
    ...
    # exp 5: trainable low-rank bigram adapter
    bigram_adapter_rank = int(os.environ.get("BIGRAM_ADAPTER_RANK", 0))
    bigram_adapter_lr = float(os.environ.get("BIGRAM_ADAPTER_LR", matrix_lr))
    bigram_adapter_init_path = resolve_repo_path(os.environ.get("BIGRAM_ADAPTER_INIT_PATH", ""))
...
def load_bigram_adapter_init(
    ...
    if rank <= 0:
        raise ValueError(f"BIGRAM_ADAPTER_RANK must be positive, got {rank}")
    if rank > vocab_size:
        raise ValueError(f"BIGRAM_ADAPTER_RANK={rank} exceeds vocab_size={vocab_size}")

    matrix = load_dense_bigram_matrix(path=path, vocab_size=vocab_size, matrix_type="logp")
    ...
    sqrtS = torch.sqrt(torch.clamp(S, min=0.0))
    A = U * sqrtS.unsqueeze(0)
    B = Vh.transpose(0, 1) * sqrtS.unsqueeze(0)
...
class GPT(nn.Module):
    def __init__(
        ...
        bigram_adapter_init: tuple[torch.Tensor, torch.Tensor] | None = None,
    ):
        super().__init__()
    ...
    def _init_weights(self) -> None:
        ...
        if self.bigram_adapter_rank > 0:
            if self._bigram_adapter_init is not None:
                ...
                with torch.no_grad():
                    self.bigram_adapter_A.copy_(a_init.to(dtype=self.bigram_adapter_A.dtype))
                    self.bigram_adapter_B.copy_(b_init.to(dtype=self.bigram_adapter_B.dtype))
            else:
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        if self.bigram_adapter_rank > 0:
            if self.bigram_adapter_A is None or self.bigram_adapter_B is None or self.bigram_adapter_scale is None:
                raise RuntimeError("bigram adapter parameters are missing")
            adapter_rows = F.embedding(input_ids, self.bigram_adapter_A)
            adapter_logits = torch.matmul(
                adapter_rows,
                self.bigram_adapter_B.transpose(0, 1).to(dtype=adapter_rows.dtype),
            )
            logits_proj = logits_proj + self.bigram_adapter_scale.to(dtype=logits_proj.dtype) * adapter_logits
...
def main() -> None:
    ...
    if adapter_matrix_params:
        optimizer_adapter = torch.optim.AdamW(
            [{"params": adapter_matrix_params, "lr": args.bigram_adapter_lr, "base_lr": args.bigram_adapter_lr, "weight_decay": args.adam_wd}],
            betas=(args.beta1, args.beta2),
            eps=args.adam_eps,
            fused=True,
        )
